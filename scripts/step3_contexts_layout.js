    ...
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2 max-w-md w-full px-4 pointer-events-none">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-center justify-between p-4 rounded-2xl shadow-2xl border backdrop-blur-md transition-all animate-in slide-in-from-bottom-5 duration-300 ${
              toast.type === 'success'
...
save('src/app/globals.css', `@tailwind base;
@tailwind components;
@tailwind utilities;
...
save('src/app/layout.tsx', `import type { Metadata } from 'next';
import './globals.css';
...
import { Ticket, Repeat, Compass, ShieldAlert, LogOut, User, Sparkles, QrCode } from 'lucide-react';
...
export default function Navbar() {
  ...
  if (user) {
    navLinks.push({ href: '/tickets', label: 'My Passes', icon: Ticket });
  }

  if (user && (user.role === 'ORGANIZER' || user.role === 'ADMIN')) {
    navLinks.push({ href: '/organizer', label: 'Host Hub', icon: QrCode });
  }
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="flex items-center gap-3">
              <Link
                href="/dashboard"
                className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 transition-all text-xs"
              >
                <div className="w-6 h-6 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-bold text-[11px]">
                  {user.name.charAt(0)}
                </div>
                <div className="hidden sm:block text-left">
                  <p className="font-bold text-white leading-none text-xs">{user.name}</p>
                  <p className="text-[10px] text-indigo-400 font-mono">{user.college.split(' ')[0]}</p>
                </div>
              </Link>
...
import Link from 'next/link';
import { Ticket, ShieldCheck, Lock, Heart } from 'lucide-react';
...
// 7. src/components/common/DemoSwitcher.tsx
